...
int takeBlockControl(int increase[]);				//블록 조작 방향 입력 받음			Taking input for moving blocks
...
void checkNumber(int, int, int increase[]);		// 블록 인접 블록숫자 확인			Checking number of adjacent block
...
void main() {
	...
	while (1) {
		...
		for (int j = 0; j < 5; j++) {
			if (takeBlockControl(increase) == SPACE) break;
			printGameScreen();
		...
		if (!block.isactive && !block2.isactive) { // 이 부분은 떨어지는 블록이 바닥이나 다른블록에 닿았는지 체크합니다. This statement is cheking that wether the falling blocks got touched by floor or other blocks.
			checkNumber(block.pos_x, block.pos_y, increase);
			if (isStageEnd() != 1)
				newBlock();
		}
	}
...
int getRandomNumber() {
	...
	*/
	int numbers[10] = { 2,2,2,2,2,4,4,4,8,8 };
	int rmd = rand() % 10;
...
int takeBlockControl(int increase[]) {
	...
	if (_kbhit()) {
		...
		if (input_blockControl == 224 && block.isactive && block2.isactive) { //방향키인경우  Cases for direction keys
			...
			switch (input_blockControl) {				
			case SPACE:
				while (block.isactive == 1 || block2.isactive == 1) {
					moveBlock(DOWN);
				}
				score += 5;
				return SPACE;
...
void moveBlock(int direction) { // 좌,우,아래 입력시 움직임 함수	Moving blocks for left, reight, and down input
	...
	case DOWN:
		...
		else block2.isactive = 0;
		
		while (block.isactive == 1 && block2.isactive == 0) {
			if (block.pos_x < X - 1 && gameScreen[block.pos_x + 1][block.pos_y] == 0) {
				gameScreen[block.pos_x + 1][block.pos_y] = block.num;
				gameScreen[block.pos_x][block.pos_y] = 0;
				block.pos_x++;
		...
		while (block.isactive == 0 && block2.isactive == 1) {
			if (block2.pos_x < X - 1 && gameScreen[block2.pos_x + 1][block2.pos_y] == 0) {
				gameScreen[block2.pos_x + 1][block2.pos_y] = block2.num;
				gameScreen[block2.pos_x][block2.pos_y] = 0;
				block2.pos_x++;
...
int checkAdjacentBlock(int x, int y, int increase[]) { //Merging 조건 확인 함수		Checking merging condition
	 /*
	 조건확인하여 연산하고 Merging
	  64 생성시 점수 계산
	 */
	for (int i = X - 1; i > 0; i--) {
			if (i < (X - 1) && gameScreen[i][y] != 0 && gameScreen[i + 1][y] == 0) {
				gameScreen[i + 1][y] = gameScreen[i][y];
				gameScreen[i][y] = 0;
				if (i < (X - 1) && gameScreen[i][y + 1] != 0 && gameScreen[i + 1][y + 1] == 0) {
					gameScreen[i + 1][y + 1] = gameScreen[i][y + 1];
					gameScreen[i][y + 1] = 0;
					continue;
				}
			}
	}	
	if (gameScreen[x][y - 2] > 0 && gameScreen[x][y - 2] % 2 == 0 && gameScreen[x][y - 1] % 2 == 1 && gameScreen[x][y] > 0 && gameScreen[x][y] % 2 == 0 && gameScreen[x][y + 1] % 2 == 1 && gameScreen[x][y + 2] > 0 && gameScreen[x][y + 2] % 2 == 0 && y + 1 < Y&&y - 1 >= 0) {
		if (gameScreen[x][y + 1] == '+') {
			gameScreen[x][y] = gameScreen[x][y] + gameScreen[x][y + 2];
			gameScreen[x][y + 1] = 0;	gameScreen[x][y + 2] = 0;
			if (gameScreen[x][y - 1] == '-') if (gameScreen[x][y - 2] == gameScreen[x][y]) {
				gameScreen[x][y - 2] = 999;	gameScreen[x][y - 1] = 0;	gameScreen[x][y] = 0;
			}
		}
		else if (gameScreen[x][y + 1] == '-') {
			if (gameScreen[x][y - 1] == '+') {
				gameScreen[x][y - 2] = gameScreen[x][y - 2] + gameScreen[x][y];
				gameScreen[x][y - 1] = 0;	gameScreen[x][y] = 0;
				if (gameScreen[x][y - 2] - gameScreen[x][y + 2] > 0) gameScreen[x][y - 2] = gameScreen[x][y - 2] - gameScreen[x][y + 2];
				else if (gameScreen[x][y - 2] - gameScreen[x][y + 2] < 0) gameScreen[x][y - 2] = gameScreen[x][y + 2] - gameScreen[x][y - 2];
				else gameScreen[x][y - 2] = 999;
			}
			else if (gameScreen[x][y - 1] == '-') {
				if (gameScreen[x][y - 2] > gameScreen[x][y]) gameScreen[x][y - 2] -= gameScreen[x][y];
				else if (gameScreen[x][y - 2] < gameScreen[x][y]) gameScreen[x][y - 2] = gameScreen[x][y] - gameScreen[x][y - 2];
				gameScreen[x][y - 1] = 0;	gameScreen[x][y] = 0;
				if (gameScreen[x][y - 2] > gameScreen[x][y + 2]) gameScreen[x][y - 2] -= gameScreen[x][y + 2];
				else if (gameScreen[x][y - 2] == gameScreen[x][y + 2]) gameScreen[x][y - 2] = 999;
				else gameScreen[x][y - 2] = gameScreen[x][y + 2] - gameScreen[x][y - 2];
			}
			gameScreen[x][y + 1] = 0;	gameScreen[x][y + 2] = 0;
		}
	}
	if (gameScreen[x][y] == '+') {
		if (gameScreen[x][y - 1] > 0 && gameScreen[x][y - 1] % 2 == 0 && gameScreen[x][y + 1] > 0 && gameScreen[x][y + 1] % 2 == 0 && y + 1 < Y&&y - 1 >= 0) {
		...
		if (gameScreen[x][y - 1] > 0 && gameScreen[x][y - 1] % 2 == 0 && gameScreen[x][y + 1] > 0 && gameScreen[x][y + 1] % 2 == 0 && y + 1 < Y&&y - 1 >= 0) {
			if (gameScreen[x][y - 1] - gameScreen[x][y + 1] > 0)gameScreen[x][y - 1] = gameScreen[x][y - 1] - gameScreen[x][y + 1];
			else if (gameScreen[x][y - 1] == gameScreen[x][y + 1]) gameScreen[x][y - 1] = 999;
			else gameScreen[x][y - 1] = gameScreen[x][y + 1] - gameScreen[x][y - 1];
			gameScreen[x][y] = 0;
			gameScreen[x][y + 1] = 0;
			return 1;
		}
	...
	if (gameScreen[x][y] == '+') {
		if (gameScreen[x + 1][y] > 0 && gameScreen[x + 1][y] % 2 == 0 && gameScreen[x - 1][y] > 0 && gameScreen[x - 1][y] % 2 == 0 && x + 1 < X) {
			if (gameScreen[x - 1][y + 2] > 0 && gameScreen[x - 1][y + 2] % 2 == 0 && gameScreen[x - 1][y + 1] % 2 != 0) {
				if (gameScreen[x - 1][y + 1] = '+') {
					gameScreen[x - 1][y] += gameScreen[x - 1][y + 2];
					gameScreen[x - 1][y + 1] = 0;	gameScreen[x - 1][y + 2] = 0;
				}
				else if (gameScreen[x - 1][y + 1] = '-') {
					gameScreen[x - 1][y] = fabs(gameScreen[x - 1][y + 2] - gameScreen[x - 1][y]);
					gameScreen[x - 1][y + 1] = 0;	gameScreen[x - 1][y + 2] = 0;
				}
			}
			else if (gameScreen[x - 1][y - 2] > 0 && gameScreen[x - 1][y - 2] % 2 == 0 && gameScreen[x - 1][y - 1] % 2 != 0) {
				if (gameScreen[x - 1][y - 1] = '+') {
					gameScreen[x - 1][y - 2] += gameScreen[x - 1][y];
					gameScreen[x - 1][y - 1] = 0;	gameScreen[x - 1][y] = 0;
				}
				else if (gameScreen[x - 1][y - 1] = '-') {
					gameScreen[x - 1][y - 2] = fabs(gameScreen[x - 1][y - 2] - gameScreen[x - 1][y]);
					gameScreen[x - 1][y - 1] = 0;	gameScreen[x - 1][y - 2] = 0;
				}
			}
		...
		if (gameScreen[x + 1][y] > 0 && gameScreen[x + 1][y] % 2 == 0 && gameScreen[x - 1][y] > 0 && gameScreen[x - 1][y] % 2 == 0 && x + 1 < X) {
			...
			if (gameScreen[x - 1][y] - gameScreen[x + 1][y] > 0) gameScreen[x - 1][y] = gameScreen[x - 1][y] - gameScreen[x + 1][y];
			else if (gameScreen[x + 1][y] == 0) gameScreen[x + 1][y] = '*';
			else gameScreen[x + 1][y] = gameScreen[x + 1][y] - gameScreen[x - 1][y];
			gameScreen[x][y] = 0;
			gameScreen[x - 1][y] = 0;
			return 1;
		}
...
void checkNumber(int x, int y, int increase[]) {
	checkAdjacentBlock(x, y, increase);
	printGameScreen();
	Sleep(200);
	for (int i = X - 1; i > 0; i--) {
		for (int j = 0; j < Y; j++) {
			if (checkAdjacentBlock(i, j, increase)) {
				printGameScreen();
				Sleep(200);
				i = X;
				break;
...
void setGameFlow(int setGameFlowType) { // Implement this function to set conditions of stages.  스테이지의 조건들을 설정하기위해 이 함수를 완성하세요.
	...
	default:
		clearScreen();
		setCoordinate(MAP_ADJ_X + 2, MAP_ADJ_Y + 13); 
		printf(" @System error: state end condition currupted.");
		exit(0);
	}
}
